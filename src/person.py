...
class tracking():
	#init
	def __init__(self):
		self.nothing = rospy.Publisher("/nothing",Int32,queue_size=1)
		self.person = rospy.Publisher("/person",numpy_msg(Floats),queue_size=1)
		
		rospy.Subscriber("scan2", LaserScan, self.processing_laser)
	...
	def processing_laser(self,data):
		...
		#−120°から-120°まで
		laser_range = laser_range[43:]#40から43に変更2022/7/23
	...
	def recognition(self):
		person_center = np.array([[0.35,0.0],[0.35,0.0],[0.35,0.0]])
		...
		while not rospy.is_shutdown():
			if self.laser_xy.size > 0:
				...
				start_index_max = start_index[index_max]
				end_index_max = end_index[index_max]
				start_index_sec = start_index[index_sec]
				end_index_sec = end_index[index_sec]
				...
				print(legs_pdf[index_max])

				if legs_pdf[index_max]>20.0: #足が見つかった場合
					print("find")
					n = 0
					a = 0
					person_center = np.array([center_max,center_max,center_max])
					if leg_width_max < 0.18 and legs_pdf[index_sec]>10.0:
						#[人の重心,片足(重さ１番目),片足(重さ２番目)]
						person_center = np.array([center,center_max,center_sec])
						print(legs_pdf[index_sec])
				...
				print(person_center_f)
				self.person.publish(person_center_f)
				self.nothing.publish(n)
